...
- **Static shape check** — confirm an allowlist fix is the whole-command `:*`
  ...
  a subshell, an absolute path outside the worktree, an unexpanded shell
  metachar (`$VAR`, unescaped `*` — escaping with `\$` doesn't help; the
  matcher reads raw text), or a full-path invocation, no allow entry will
  rescue it — the gate must be rewritten.
