...
|---|---|---|---|---|
| 1 | `:*` after a command name allowlists "any suffix" | `setup-ralph/SKILL.md` step 3; common template entries like `Bash(git:*)` | A1–A5 | confirmed — A1–A5 Allowed under `Bash(rmdir:*)` across bare, single positional, multiple positionals, quoted-with-space, and leading-whitespace shapes (2026-05-23). Multi-word side also confirmed via Bm9–Bm11 (word-boundary on both edges) and Bm16 (position-locked at argv 0/1). §2 path-locality remains the boundary: B3 (`rmdir /etc`) Denied under explicit allow rule (2026-05-23) — §2 fires for path-typed commands regardless of how the allow was obtained. |
| 2 | A bare command name with no `:*` is an exact match | `Bash(date +%s)` in the template uses this shape | Cr1–Cr9 (single-word `yprobe`); C5–C9 (multi-word `xprobe two`) | confirmed — Cr1 Allowed; Cr2–Cr9 Denied across every suffix shape probed (flag, multi-positional, empty quoted, double whitespace, outside-cwd path, cwd-relative). C5 Allowed; C6–C9 Denied across flag suffix, positional suffix, bare leader, and different-subcommand. Both confirmations 2026-05-23 under fictional `yprobe` / `xprobe two` targets, which coexist with `Bash(rmdir:*)` and `Bash(xprobe one:*)` in S1 without rule shadowing. |
| 3 | Compound shapes (`&&` / `||` / `;` / pipes / redirects / subshells) are distinct patterns from their parts and fail when not explicitly allowlisted | `ORCHESTRATOR.md` "Bash command shape"; `PROMPT.md` "Bash command shape" | D1–D6 (single-word `rmdir`); E1–E12 (mixed) | confirmed — every compound shape probed Allows iff every stage independently clears allow + safe list + §2, and Denies otherwise. Single-word leading stage: D1–D6 + E1–E6 (2026-05-23) cover `&&`, `;`, `||`, background, pipe, redirect, subshell, backtick. Multi-word leading stage: E7–E12 (2026-05-23) — pipe with safe-listed tail Allowed, pipe with env Denied, `&&` with env Denied, both halves matching different rules Allowed, §2 fires on downstream `cat`. Subshell rejection (E5/E6) re-confirmed (Finding §5). |
| 4 | The matcher decomposes `&&`-chained compounds and checks each half against allow + deny | `ORCHESTRATOR.md` step 6 implicitly; permission-denied worker doctrine | D2 (deny on right), E10 (allow-missing on right) | confirmed — D2 Denied (deny rule on right half: `rmdir probe-d2-dne && cd .` hits `Bash(cd:*)` deny, distinct "with command …" message form); E10 Denied (allow rule missing on right half: `xprobe one && env` with no env allow rule). Together rule out "matcher treats `&&` as one shape" via both denial mechanisms (2026-05-23). |
| 5 | A flag-bearing variant of an allowlisted command (e.g. `rmdir --help`) is matched by `Bash(<cmd>:*)` | various template entries assume this | B1, B2, B4; Bm1–Bm17 | confirmed at both levels. Multi-word level via Bm1–Bm6 (`:*` accepts flags, `=`-suffixed flags, multiple flags); Bm9–Bm11 confirm strict word-boundary on both sides; Bm14 confirms §2 is command-specific; Bm16 confirms position-locking at argv 0/1. Single-word level via B1 (flag), B2 (flag + positional), B4 (cwd-relative positional) Allowed under `Bash(rmdir:*)` (2026-05-23); B3 (`rmdir /etc`) Denied confirms §2 fires under explicit allow. |
